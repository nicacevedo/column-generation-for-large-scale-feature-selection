...
# Size of the data
m_real = [(i+1)*1000 for i in range(10)]
n_cols = [int(np.round(x*(1-per_error))) for x in m_real]
n_obs = 10000

# New version
def original_data_generation(m_real, n_cols, n_obs, random_state=123):
    ...
    for i,n_var in enumerate(m_real):
        ...
        y = pd.DataFrame(np.round(X @ betas, 3))
        ...
        # Original
        np.savetxt(f'./BasesNew/synthetic_X_{m_real[i]}_{n_obs}_e{int(100*per_error)}.csv', X, delimiter=',', fmt='%.3f')
        np.savetxt(f'./BasesNew/synthetic_y_{m_real[i]}_{n_obs}_e{int(100*per_error)}.csv', y, delimiter=',', fmt='%.3f')

        # Save the noise columns for the new data
        np.savetxt(f'./BasesNew/Noise/synthetic_noise_columns_{m_real[i]}_{n_obs}_e{int(100*per_error)}.csv', np.sort(noise_columns), delimiter=',', fmt='%d')


original_data_generation(m_real, n_cols, n_obs, random_state=123)


def new_data_generation(n_real, n_cols, n_obs, random_state=123):
    n_var_errors = [n_cols[i] - n_real[i] for i in range(len(n_cols))] 
    ...
    for i,n_var in enumerate(n_cols):
        ...
        # For analysis
        np.savetxt(f'./Synthetic Data Analysis/Bases/syntetic_X_{n_real[i]}_{n_obs}.csv', X, delimiter=',', fmt='%.3f')
        np.savetxt(f'./Synthetic Data Analysis/Bases/syntetic_y_{n_real[i]}_{n_obs}.csv', y, delimiter=',', fmt='%.3f')
