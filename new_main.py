        ...
        # for n_row in [10000]:#[91992]: # casen
            ...
            for m_col in [(2*i+1)*1000 for i in range(0,3)]:
                ...
                if real_data:
                    ...
                    # df = pd.read_csv('./real-data/casen_full_ingreso_num_interact_g2.csv')
                    # X = df.drop(columns=['ingreso']).to_numpy()
                    # y = df['ingreso'].to_numpy()

                    # # Delete columns with no variance (16 and 17)
                    # X = np.delete(X, [14, 15, 16,17,18,19, 387], axis=1)

                    # # Random select 40k rows
                    # idx = np.random.choice(X.shape[0], n_row, replace=False)
                    # X = X[idx,:m_col]
                    # y = y[idx]
                    ...
                    # X = pd.read_fwf('real-data/dorothea/DOROTHEA/dorothea_train.data', header=None)
                    # X2 = []
                    # # all the data is in one column (0), we need to split it and convert it to int
                    # for i in range(X.shape[0]):
                    #     print(len(X.loc[i,0].splits(' ')))
                    #     X2.append(X.loc[i,0].split(' '))
                    # # X2 = pd.DataFrame(X2)
                    # # print(X2.head())
                    # # # print(np.array(X.loc[0,0].split(' '), dtype=int) )
                    # # # print(X)
                    # # # X = pd.DataFrame(X[0].str.split(' '))
                    # # y = pd.read_csv('real-data/dorothea/DOROTHEA/dorothea_train.labels', header=None)
                ...
                for tau_exp in np.linspace(-2, 0.1, 10): # -1/.15 for the other synthetic
