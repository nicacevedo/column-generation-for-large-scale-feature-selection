...
def SCIKIT_LASSO(X,y, tau, solver=cp.MOSEK, solver_params={}, solver_verbose=False, lasso_type='Lasso', tol=1e-6):
    ...
    # objective related values
    xi = np.linalg.norm(y - X @ beta)
    lasso_value = xi**2 + 2*tau * np.linalg.norm(beta, 1)

    return beta, z, lasso_value, (t1-t0)/60, (t1p-t0p)/60, {'k':clf.n_iter_}
...
def SOCP(X,y,tau, kappa, eps_sqrt=0, solver=cp.MOSEK, solver_params={}, solver_verbose=False):
    print("""
    --------------------------------------------------
                        SOCP Model
    --------------------------------------------------
...
def SOCP_v2(X,y,tau, kappa, theta=0, eps_sqrt=0, solver=cp.MOSEK, solver_params={}, solver_verbose=False):
    print("""
    --------------------------------------------------
                        SOCP Model
    --------------------------------------------------
